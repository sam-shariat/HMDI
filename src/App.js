...
const App = function AppWrapper() {
  const [address, setAddress] = useState(
    localStorage.getItem("algo_wallet_address")
  );
    ...
    <>
      ...
      {address && Number(address) !== 0 ? (
        <Container fluid className="m-0 p-0">
          <Container fluid="md">
            <Nav className="justify-content-between pt-3">
              <Nav.Item>
                <Nav.Link href="/HMDI/" className="p-0 m-0 py-1">
                  <h3
                    style={{ cursor: "pointer" }}
                    className="fw-bold d-flex m-0"
                  >
                    <ReactSVG src={coverImg} />
                    {isMobile ? "HMDI" : "HelpMeDoit"}
                  </h3>
                </Nav.Link>
              </Nav.Item>
              <Nav.Item>
          ...
          <main>
            {appId ? (
              <SingleProduct address={address} fetchBalance={fetchBalance} />
            ) : (
        ...
        <Cover
          ...
          sub={
            "A Decentralized Platform to Raise Funds For Your Most Important Project(s) and Donate Some ALGO(s) To Help Your Fellows In Need"
          }
          coverImg={coverImg}
